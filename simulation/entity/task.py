...
from simulation.entity.cnn_model import AlexNet
...
def getTaskInstanceByName(name: str) -> Task:
    for task_class in UI_OPTIONS:
        if task_class.__name__ == name:
            return task_class()

def getTaskClasses(selected_task_list: list[str]) -> list[Task]:
