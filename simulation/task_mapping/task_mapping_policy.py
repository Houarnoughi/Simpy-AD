...
class CustomTaskMappingPolicy(TaskMappingPolicy):
    ...
    def convertToFeatureVector(self, task: 'Task', pu: 'ProcessingUnit'):
        ...
        raise ValueError("offload time to impl")
        offload_time = 0 #config.NETWORK.getTransferDuration(task.getSize())
...
def getMappingInstanceByName(name: str) -> TaskMappingPolicy:
    for mapping_class in UI_OPTIONS:
        if mapping_class.__name__ == name:
            return mapping_class()

def getTaskMapperClass(name: str) -> TaskMappingPolicy:
